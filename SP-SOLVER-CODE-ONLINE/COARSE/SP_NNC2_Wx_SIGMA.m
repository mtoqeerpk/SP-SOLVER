...
%   A(n,n)=0;
    ...
    %NW corner*******from top view NW corner
               ...
               elseif (i==DX3D && j==1) && k==1
                  ...
                  a_Wx1 = -sigma(j,i,k)*sigma(a(1),b(1),c(1))*(y(j+1)-y(j))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(x(i)-x(i-1))+sigma(a(1),b(1),c(1))*0.5*(x(i+1)-x(i)))/wsize(1);   
                  if c(2) == 0    
                      a_Wx2 = 0;   
                  else
                      a_Wx2 = -sigma(j,i,k)*sigma(a(2),b(2),c(2))*(y(j+1)-y(j))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(x(i)-x(i-1))+sigma(a(2),b(2),c(2))*0.5*(x(i+1)-x(i)))/wsize(1);   
                  end
                  if c(3) == 0      
                      a_Wx3 = 0;    
                  else
                      a_Wx3 = -sigma(j,i,k)*sigma(a(3),b(3),c(3))*(y(j+1)-y(j))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(x(i)-x(i-1))+sigma(a(3),b(3),c(3))*0.5*(x(i+1)-x(i)))/wsize(1);    
                  end
                  if c(4) == 0    
                      a_Wx4 = 0;  
                  else
                      a_Wx4 = -sigma(j,i,k)*sigma(a(4),b(4),c(4))*(y(j+1)-y(j))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(x(i)-x(i-1))+sigma(a(4),b(4),c(4))*0.5*(x(i+1)-x(i)))/wsize(1);   
                  end
                  if c(5) == 0      
                      a_Wx5 = 0;     
                  else
                      a_Wx5 = -sigma(j,i,k)*sigma(a(5),b(5),c(5))*(y(j+1)-y(j))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(x(i)-x(i-1))+sigma(a(5),b(5),c(5))*0.5*(x(i+1)-x(i)))/wsize(1);    
                  end
                  a_Ey = -sigma(j,i,k)*sigma(j+1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j+2)-y(j+1))+sigma(j+1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Ez);
                A(n,[ n (n+1) (n+(DY3D*DX3D))]) = [ a_P a_Ey a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               %max z
               elseif (i==DX3D && j==1) && k==DZ3D
                  ...
                  a_Ey = -sigma(j,i,k)*sigma(j+1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j+2)-y(j+1))+sigma(j+1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wz);
               ...
               elseif (i==DX3D && j==DY3D) && k==1
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Ez);
               ...
               elseif (i==DX3D && j==DY3D) && k==DZ3D
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Wz);
    ...
    % edges
              ...
              %NW edge*******from top view NW corner
               ...
               elseif (j==1 && k==1)
                  ...
                  a_Ey = -sigma(j,i,k)*sigma(j+1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j+2)-y(j+1))+sigma(j+1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Ez);
                A(n,[n (n+1) (n+DY3D) (n+(DY3D*DX3D))]) = [ a_P a_Ey a_Ex a_Ez]; 
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5];               
               ...
               elseif (j==1 && k==DZ3D)
                  ...
                  a_Ey = -sigma(j,i,k)*sigma(j+1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j+2)-y(j+1))+sigma(j+1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wz);
                 A(n,[(n-(DY3D*DX3D)) n (n+1) (n+DY3D)]) = [a_Wz  a_P a_Ey a_Ex];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               ...
               elseif (j==DY3D && k==1)
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Ez);
                A(n,[(n-1) n (n+DY3D) (n+(DY3D*DX3D))]) = [ a_Wy a_P a_Ex a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               ...
               elseif (j==DY3D && k==DZ3D)
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Wz);
                A(n,[(n-(DY3D*DX3D))  (n-1) n (n+DY3D)]) = [a_Wz  a_Wy a_P a_Ex];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               ...
               elseif (i==DX3D && j==1)
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Ez + a_Wz);
               ...
               elseif (i==DX3D && j==DY3D)
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Ez + a_Wz);
               ...
               elseif (i==DX3D && k==1)
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Ez);
               ...
               elseif (i==DX3D && k==DZ3D)
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Wz);
    ...
    %faces
               ...
               elseif j==1
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Ez + a_Wz);
                A(n,[(n-(DY3D*DX3D)) n (n+1) (n+DY3D) (n+(DY3D*DX3D))]) = [a_Wz  a_P a_Ey a_Ex a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               ...
               elseif j==DY3D
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Wy + a_Ez + a_Wz);
                A(n,[(n-(DY3D*DX3D)) (n-1) n (n+DY3D) (n+(DY3D*DX3D))]) = [a_Wz  a_Wy a_P a_Ex a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               %near face******from top view top face
                elseif k==1
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Ez);
                A(n,[(n-1) n (n+1) (n+DY3D) (n+(DY3D*DX3D))]) = [ a_Wy a_P a_Ey a_Ex a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               %far face******from top view bottom face                
                elseif k==DZ3D
                  ...
                  a_Wy = -sigma(j,i,k)*sigma(j-1,i,k)*(x(i+1)-x(i))*(z(k+1)-z(k))/(sigma(j,i,k)*0.5*(y(j)-y(j-1))+sigma(j-1,i,k)*0.5*(y(j+1)-y(j)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Wz);
                A(n,[(n-(DY3D*DX3D))  (n-1) n (n+1) (n+DY3D)]) = [a_Wz  a_Wy a_P a_Ey a_Ex];    
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5]; 
               ...
               elseif i==DX3D
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Ez + a_Wz);
    ...
    %non boundary
                  ...
                  a_Ez = -sigma(j,i,k)*sigma(j,i,k+1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k+2)-z(k+1))+sigma(j,i,k+1)*0.5*(z(k+1)-z(k)));
                  a_Wz = -sigma(j,i,k)*sigma(j,i,k-1)*(x(i+1)-x(i))*(y(j+1)-y(j))/(sigma(j,i,k)*0.5*(z(k)-z(k-1))+sigma(j,i,k-1)*0.5*(z(k+1)-z(k)));
                  a_P  = -(a_Ex + a_Wx1 + a_Wx2 + a_Wx3 + a_Wx4 + a_Wx5 +  a_Ey + a_Wy + a_Ez + a_Wz);    
                A(n,[(n-(DY3D*DX3D)) (n-1) n (n+1) (n+DY3D) (n+(DY3D*DX3D))]) = [a_Wz  a_Wy a_P a_Ey a_Ex a_Ez];
                A(n,[nWx1 nWx2 nWx3 nWx4 nWx5]) = [a_Wx1 a_Wx2 a_Wx3 a_Wx4 a_Wx5];   
