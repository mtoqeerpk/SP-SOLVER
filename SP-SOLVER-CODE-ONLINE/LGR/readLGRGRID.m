% function [filecontents,nnc0]=readFfilezz(filename,onlylast)
...
gridcontent.varnames=[];
gridcontent.typename=[];
...
while NI < slength % there should be something left to read
  ...
  elseif strcmp(vn,check3)
      if nnc > 1
          formatspec = 'nnc = max(%s.NNCHEAD);\n';
          eval(sprintf(formatspec,file))
      end
      file = ['nnc', int2str(nnc)];
      nnc=nnc+1;
...
%       check = 0;
  ...
  if check < 1
  gridcontent.varnames = [gridcontent.varnames; varname];
  gridcontent.typename = [gridcontent.typename; typename];
  %
  if numentries > 0
  ...
  if onlylast==0 & isfield(gridcontent,vn)
    gridcontent = setfield(gridcontent,vn,...
		       [getfield(gridcontent,vn),data]);
  else
    gridcontent = setfield(gridcontent,vn,data);
  end
...
%   formatspec = '%s.typename = [gridcontent.varnames; typename];\n';
%   eval(sprintf(formatspec,file))
